...
public class MessageRepository {
    ...
    public void getChat(String chatID, ICallback callback)
    {
        dbRef.child("server/chats/" + chatID).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                ...
                if(task.isSuccessful() && task.getResult().getValue() != null) {
                    ...
                    chat.addInfoFromSnapshot(task.getResult().child("info"));

                    messageViewModel.liveChat.setValue(chat);
    ...
    public void addMessage(String chatId, Message message, ICallback callback) {
        ...
        String messageID = dbRef.child("server/chats/" + chatId).push().getKey();

        dbRef.child("server/chats/" + chatId + "/messages/" + messageID).setValue(message, new DatabaseReference.CompletionListener() {
            @Override
    ...
    {
        dbRef.child("server/chats/" + chatID + "/messages").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                LoadingScreen.getInstance().hideLoadingScreen();
